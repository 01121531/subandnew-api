...
func mailboxImportInput(c *gin.Context) (string, []byte, string, bool, bool, bool, bool) {
	...
	if media == "multipart/form-data" {
		...
		for key, values := range c.Request.MultipartForm.Value {
			if (key != "account_type" && key != "format" && key != "ignore_extra_fields" && key != "allow_partial" && key != "update_existing") || len(values) != 1 {
		...
		updateValue := c.Request.FormValue("update_existing")
		if (ignoreValue != "" && ignoreValue != "true" && ignoreValue != "false") || (partialValue != "" && partialValue != "true" && partialValue != "false") || (updateValue != "" && updateValue != "true" && updateValue != "false") {
			mailboxBadRequest(c, "mailbox_invalid_request")
			return "", nil, "", false, false, false, false
		}
		...
		return format, data, accountType, ignoreValue == "true", partialValue == "true", updateValue == "true", true
	}
	var input struct {
	...
	return input.Format, []byte(input.Text), accountType, input.IgnoreExtraFields, input.AllowPartial, input.UpdateExisting, ok
}
func ImportMailboxAccounts(preview bool) gin.HandlerFunc {
	return func(c *gin.Context) {
		format, data, accountType, ignoreExtra, allowPartial, updateExisting, ok := mailboxImportInput(c)
		if !ok {
			return
		}
		service := mailboxService().WithImportExtraFields(ignoreExtra).WithPartialImport(allowPartial).WithImportUpdateExisting(updateExisting)
		if preview {
