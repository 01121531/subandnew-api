...
func registerManagedInstanceRoutes(apiRouter *gin.RouterGroup) {
	schedules := apiRouter.Group("/managed-account-export-schedules")
	schedules.Use(middleware.AdminAuth(), middleware.RequirePermission(authz.ManagedInstanceUsageView))
	schedules.GET("", controller.ListAccountExportSchedules)
	schedules.POST("", controller.SaveAccountExportSchedule)
	schedules.GET("/:id", controller.GetAccountExportSchedule)
	schedules.PUT("/:id", controller.SaveAccountExportSchedule)
	schedules.DELETE("/:id", controller.AccountExportScheduleAction)
	schedules.POST("/:id/:action", controller.AccountExportScheduleAction)
	schedules.GET("/:id/runs", controller.ListAccountExportScheduleRuns)
	schedules.POST("/:id/deliveries/:delivery_id/retry", controller.RetryAccountExportDelivery)
	...
	unifiedExportGroup := apiRouter.Group("/managed-exports")
	unifiedExportGroup.Use(middleware.AdminAuth(), middleware.RequirePermission(authz.ManagedInstanceUsageView))
	unifiedExportGroup.GET("", controller.ListManagedUsageExports)
	unifiedExportGroup.GET("/:task_id", controller.GetManagedUsageExport)
	unifiedExportGroup.GET("/:task_id/warnings", controller.GetManagedUsageExportWarnings)
	unifiedExportGroup.GET("/:task_id/download", controller.DownloadManagedUsageExport)
	unifiedExportGroup.POST("/:task_id/cancel", controller.CancelManagedUsageExport)
	unifiedExportGroup.POST("/:task_id/retry", controller.RetryManagedUsageExport)
	unifiedExportGroup.DELETE("/:task_id", controller.DeleteManagedUsageExport)
	...
	exportGroup.GET("/:task_id/warnings", controller.GetManagedUsageExportWarnings)
...
var managedInstancePermissionRoutes = []permissionRoute{
	...
	{method: http.MethodGet, path: "/dashboard-snapshots", permission: authz.ManagedInstanceView, handler: controller.GetManagedDashboardSnapshots},
	...
	{method: http.MethodGet, path: "/:id/conductor/key-usage", permission: authz.ManagedInstanceUsageView, handler: controller.GetManagedInstanceConductorKeyUsage},
	{method: http.MethodGet, path: "/:id/account-management/snapshot", permission: authz.ManagedInstanceUsageView, handler: controller.GetManagedInstanceAccountManagementSnapshot},
	{method: http.MethodPost, path: "/:id/account-management/refresh", permission: authz.ManagedInstanceUsageView, handler: controller.RefreshManagedInstanceAccountManagement},
